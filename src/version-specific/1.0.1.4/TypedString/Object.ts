...
export function Object_generate_typescript_data(
	...
	const $ref_instance = schema_parser.types.find(
		(maybe): maybe is $ref => $ref.is_a(maybe),
	);

	if (undefined == $ref_instance) {
		throw new Error('Could not find $ref instance!');
	}
	...
	const regex = `${
		Object.keys(schema.properties)
			.map((property) => {
				...
				if ('$ref' in resolved && $ref.is_supported_$ref(resolved)) {
					const check_resolved = $ref_instance.resolve_def(
						resolved,
						resolved.$defs || {},
					);

					if (
						object_has_property(check_resolved, 'type')
						&& 'string' === check_resolved.type
						&& !object_has_property(
							check_resolved,
							'typed_string',
						)

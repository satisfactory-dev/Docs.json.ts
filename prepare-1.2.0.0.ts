...
await handler.read((data, errors) => {
	if (
		Array.isArray(data)
		&& data.length === release_1_2.maxItems
	) {
		let passing = 0;

		for (let i = 0; i < release_1_2.maxItems; ++i) {
			const item: unknown = data[i];

			const validator = ajv.compile<NativeClass>(Type.maybe_add_$defs(
				release_1_2,
				release_1_2.prefixItems[i],
			));
		...
		const percentage = (passing / release_1_2.maxItems) * 100;
...
console.log(`1.2.0.0 ${lang} updated`);

...
declare type TypeNodeGeneration_generator<T extends {[key: string]: any}> = (
	data: T
) => TypeNodeGenerationResult;

export class TypeNodeGeneration<T extends {[key: string]: any}> {
	readonly schema: Schema;
	private validate: WeakMap<Ajv, ValidateFunction<T>> = new WeakMap<
		Ajv,
		ValidateFunction<T>
	>();
...
export class TypeNodeGenerationMatcher {
	...
	find(ajv: Ajv, property: object): TypeNodeGenerationResult {
		let match =
	...
	): {[K in keyof typeof properties]: TypeNodeGenerationResult} {
...
export function create_constructor_args<T1 = string>(
	...
	const properties = required
		...
		.reduce(
			(was, is) => {
				was[is] = ts.factory.createKeywordTypeNode(

...
ImportTracker.set_imports('common/color.ts', [
	{
		import_these: ['integer_string', 'decimal_string'],
	...
	constructor_body.push(
		ts.factory.createExpressionStatement(
			ts.factory.createCallExpression(
				...
				undefined,
				(Object.keys(data.properties) as ('R' | 'G' | 'B' | 'A')[])
					.map(
						(property): ['R' | 'G' | 'B' | 'A', ts.Identifier] => {
							return [
								property,
...
export const generators = [
	new TypesGenerationMatchesReferenceName<
		...
		'color--base'
	>(['color--base'], (data, reference_name): ClassDeclaration => {

...
import {
	configure_parser,
} from './src/version-specific/1.2.2.0/SchemaParser.ts';
...
await handler.read((data, errors) => {
	if (
		Array.isArray(data)
		&& data.length === release_1_2_2_0.maxItems
	) {
		let passing = 0;

		for (let i = 0; i < release_1_2_2_0.maxItems; ++i) {
			const item: unknown = data[i];

			const validator = ajv.compile<NativeClass>(Type.maybe_add_$defs(
				release_1_2_2_0,
				release_1_2_2_0.prefixItems[i],
			));
		...
		const percentage = (passing / release_1_2_2_0.maxItems) * 100;

...
import update4_data from './data/0.4.2.11/Docs/Docs.utf8.json' with {
	type: 'json',
};
...
import {
	filenames_by_$id,
} from './src/version-specific/0.4.2.11/filenames.ts';
...
console.log(
	`update 4: ${
		100
		* (
			update4.prefixItems.filter((maybe) => !('type' in maybe)).length
			/ update4.prefixItems.length
		)
	}%`,
);
